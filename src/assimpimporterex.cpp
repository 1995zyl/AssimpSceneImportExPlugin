...
QStringList AssimpImporterEx::assimpSupportedFormatsList = AssimpImporterEx::assimpSupportedFormats();

/*!
 * Returns a QStringlist with the suffixes of the various supported asset formats.
 */
QStringList AssimpImporterEx::assimpSupportedFormats()
{
    QStringList formats;
    ...
    formats.append(QStringLiteral("glb"));
...
class AssimpTextureImageByStb : public QAbstractTextureImage
{
	Q_OBJECT
public:
	explicit AssimpTextureImageByStb(QNode* parent = 0);
...
private:
	...
	class AssimpTextureImageByStbFunctor : public QTextureImageDataGenerator
	{
	public:
		explicit AssimpTextureImageByStbFunctor(const QImage& img);

		QTextureImageDataPtr operator()() final;
		bool operator ==(const QTextureImageDataGenerator& other) const final;

		QT3D_FUNCTOR(AssimpTextureImageByStbFunctor)
	private:
        QImage m_img;
	};
};
...
/*!
 *  Constructor. Initializes a new instance of AssimpImporterEx.
 */
AssimpImporterEx::AssimpImporterEx() : QSceneImporter(),
    m_sceneParsed(false),
    m_scene(nullptr)
{
}
 ...
 */
AssimpImporterEx::~AssimpImporterEx()
{
    cleanup();
}
 ...
 */
bool AssimpImporterEx::areAssimpExtensions(const QStringList &extensions)
{
    for (const auto &ext : std::as_const(extensions))
        if (AssimpImporterEx::assimpSupportedFormatsList.contains(ext.toLower()))
            return true;
    return false;
}
 ...
 */
void AssimpImporterEx::setSource(const QUrl &source)
{
 ...
 */
void AssimpImporterEx::setData(const QByteArray &data, const QString &basePath)
{
 ...
 * the assimp parser.
 */
bool AssimpImporterEx::areFileTypesSupported(const QStringList &extensions) const
{
    return AssimpImporterEx::areAssimpExtensions(extensions);
}
 ...
 */
Qt3DCore::QEntity *AssimpImporterEx::scene(const QString &id)
{
    ...
    m_texureByStb.clearStbTextureData();
 ...
 */
Qt3DCore::QEntity *AssimpImporterEx::node(const QString &id)
{
    if (m_scene == nullptr || m_scene->m_aiScene == nullptr)
        return nullptr;
    parse();
 ...
 */
Qt3DCore::QEntity *AssimpImporterEx::node(aiNode *node)
{
    if (node == nullptr)
        return nullptr;
 ...
 */
void AssimpImporterEx::readSceneFile(const QString &path)
{
    cleanup();
    ...
    m_scene->m_importer->SetIOHandler(new AssimpHelperEx::AssimpIOSystemEx());
 ...
 * from eventual previous parsings.
 */
void AssimpImporterEx::readSceneData(const QByteArray& data, const QString &basePath)
{
    Q_UNUSED(basePath);
    cleanup();
 ...
 * \sa Material
 */
QMaterial *AssimpImporterEx::loadMaterial(uint materialIndex)
{
 ...
 */
QGeometryRenderer *AssimpImporterEx::loadMesh(uint meshIndex)
{
 ...
 * \sa Texture
 */
QAbstractTexture *AssimpImporterEx::loadEmbeddedTexture(uint textureIndex)
{
 ...
 */
QAbstractLight *AssimpImporterEx::loadLight(uint lightIndex)
{
 ...
 */
Qt3DCore::QEntity *AssimpImporterEx::loadCamera(aiNode *node)
{
...
// OPTIONAL
void AssimpImporterEx::loadAnimation(uint animationIndex)
{
 ...
 */
void AssimpImporterEx::copyMaterialName(QMaterial *material, aiMaterial *assimpMaterial)
{
    aiString name;
 ...
 */
void AssimpImporterEx::copyMaterialColorProperties(QMaterial *material, aiMaterial *assimpMaterial)
{
    aiColor3D color;
 ...
 */
void AssimpImporterEx::copyMaterialBoolProperties(QMaterial *material, aiMaterial *assimpMaterial)
{
    int value;
...
void AssimpImporterEx::copyMaterialShadingModel(QMaterial *material, aiMaterial *assimpMaterial)
{
    Q_UNUSED(material);
...
void AssimpImporterEx::copyMaterialBlendingFunction(QMaterial *material, aiMaterial *assimpMaterial)
{
    Q_UNUSED(material);
    Q_UNUSED(assimpMaterial);
    // TO DO
 ...
 *
 */
void AssimpImporterEx::copyMaterialTextures(QMaterial *material, aiMaterial *assimpMaterial)
{
    ...
    for (unsigned int i = 0; i < sizeof(textureType)/sizeof(textureType[0]); i++) {
        ...
        if (assimpMaterial->GetTexture(textureType[i], 0, &path) == AI_SUCCESS) {
            ...
            QAbstractTextureImage* texImage = nullptr;
            const aiTexture* assimpTexture = m_scene->m_aiScene->GetEmbeddedTexture(path.C_Str());
            if (assimpTexture)
            {
                texImage = new AssimpTextureImageByStb();
                if (AssimpTextureImageByStb* rawTextureImage = static_cast<AssimpTextureImageByStb*>(texImage))
                {
					bool iscompressed = assimpTexture->mHeight == 0;
					uint textureSize = assimpTexture->mWidth * (iscompressed ? 1 : assimpTexture->mHeight);
                    rawTextureImage->setImage(m_texureByStb.getStbTextureImage(reinterpret_cast<unsigned char*>(assimpTexture->pcData), textureSize));
                }
            }
            else
            {
                ...
                texImage = QAbstractNodeFactory::createNode<QTextureImage>("QTextureImage");
                if (QTextureImage* qTexImage = dynamic_cast<QTextureImage*>(texImage))
                {
                    qTexImage->setSource(QUrl::fromLocalFile(fullPath));
                    qTexImage->setMirrored(false);
                }
 ...
 */
void AssimpImporterEx::copyMaterialFloatProperties(QMaterial *material, aiMaterial *assimpMaterial)
{
    float value = 0;
...
AssimpTextureImageByStb::AssimpTextureImageByStb(QNode* parent)
	: QAbstractTextureImage(parent)
{
}

QTextureImageDataGeneratorPtr AssimpTextureImageByStb::dataGenerator() const
{
	return QTextureImageDataGeneratorPtr(new AssimpTextureImageByStbFunctor(m_img));
}

void AssimpTextureImageByStb::setImage(const QImage& img)
{
	if (img != m_img) {
		m_img = img;
		notifyDataGeneratorChanged();
...
AssimpTextureImageByStb::AssimpTextureImageByStbFunctor::AssimpTextureImageByStbFunctor(const QImage& img)
	: QTextureImageDataGenerator()
	, m_img(img)
{
}

QTextureImageDataPtr AssimpTextureImageByStb::AssimpTextureImageByStbFunctor::operator()()
{
	QTextureImageDataPtr dataPtr = QTextureImageDataPtr::create();
	dataPtr->setImage(m_img);
	return dataPtr;
}

bool AssimpTextureImageByStb::AssimpTextureImageByStbFunctor::operator ==(const QTextureImageDataGenerator& other) const
{
	const AssimpTextureImageByStbFunctor* otherFunctor = functor_cast<AssimpTextureImageByStbFunctor>(&other);
	return (otherFunctor != nullptr && otherFunctor->m_img == m_img);
}

AssimpImporterEx::SceneImporter::SceneImporter()
...
#include "moc_assimpimporterex.cpp"

#include "assimpimporterex.moc"
